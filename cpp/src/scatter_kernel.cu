...
__global__ void scatter_peakpts_kernel(const int32_t num_groups,
                                        const uint8_t num_feat,
                                        const float* __restrict__ inputs_ptr,
                                        const int32_t* __restrict__ offsets_ptr,
                                        const float* __restrict__ querypts_ptr,
                                        // output: store the not empty points for each ray
                                        float* __restrict__ outputs_ptr) {
    CUDA_GET_THREAD_ID(group_id, num_groups);
    ...
    for (uint8_t j=0; j<num_feat; ++j){
        outputs_ptr[group_id*num_feat + j] = querypts_ptr[max_idx*num_feat + j];
    }
}
...
Tensor scatter::scatter_peakpts(const Tensor& inputs, const Tensor& offsets, const Tensor& querypts) {
    // check
    CHECK_INPUT(inputs);
    CHECK_INPUT(offsets);
    CHECK_INPUT(querypts);

    const int32_t num_groups = offsets.size(0) - 1;
    const int32_t num_length = inputs.size(0);
    const int32_t num_feat = querypts.size(1);
    const int32_t num_blocks = CUDA_N_BLOCKS_NEEDED(num_groups, NUM_THREADS);

    Tensor outputs = torch::zeros({num_groups, num_feat},
                                  torch::TensorOptions().dtype(torch::kFloat).device(torch::kCUDA));

    scatter_peakpts_kernel<<<num_blocks, NUM_THREADS>>>(num_groups, num_feat,
                                                            inputs.data_ptr<float>(),
                                                            offsets.data_ptr<int32_t>(),
                                                            querypts.data_ptr<float>(),
                                                            // Output
                                                            outputs.data_ptr<float>());
