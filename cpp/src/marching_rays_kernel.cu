...
#pragma unroll 3
    for (int32_t i = 0; i < 3; ++i) {
        if (ray.dir[i] != 0.f) {
            ...
            // ray.tmin = 0.f;
            // ray.tmax = 2.f;
            // printf("ray t1:%f, t2:%f, min:%f, max:%f, world_step:%f \n", t1,
            // t2, ray.tmin, ray.tmax, ray.world_step);
        }
    }
    // printf("grid res %d, grdi_sampl_res %d \n", opt.grid_res,
    // opt.grid_sample_res);
}
...
#pragma unroll 3
        ...
        if (skip >= ray.world_step) {
            ...
            // t += fmaxf(floorf(skip / ray.world_step), 1.0) * ray.world_step;
