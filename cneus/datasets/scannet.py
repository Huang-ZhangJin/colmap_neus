...
class ScanNetDataset(DatasetBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if split == "train":
            ...
            mvs_mask_suffix = mvs_mask_suffix_TnT if ("trip" in mvs_root or  "noin" in mvs_root) else "planar_mask_mvs_200"
    ...
    def gen_rays_scannet_neus(self, factor: int = 1) -> None:
        ...
        self.rays_init = GtRays(
            ...
            vpsplanar_mask=self.planar_mask.view(-1),
            # vpsplanar_mask=None,
            mask=None,
        )
    ...
    def gen_rays_scannet_neus_keeppatch(self, factor: int = 1) -> None:
        ...
        ###########################################################
        # """ debug code """
        # ori = origins[:, 10:-10, 10:-10, :].reshape(-1, 3)
        # dir = dirs[:, 10:-10, 10:-10, :].reshape(-1, 3)
        # # dir = (dirs*depth_ratio[None, ...])[:, 10:-10, 10:-10, :].reshape(-1, 3)
        # dep = depth_gt[:, 10:-10, 10:-10, :].reshape(-1, 1)
        # dep_mask = (dep > 0).squeeze()
        # pts = ori[dep_mask] + dep[dep_mask] * dir[dep_mask]
        # norm = (
        #     self.normal_gt[:, 10:-10, 10:-10, :].reshape(-1, 3)[dep_mask]
        #     if self.input_normal
        #     else torch.zeros_like(pts)
        # )
        # print("save depth pts")
        # np.savetxt(
        #     "depthpts_patch.xyz",
        #     torch.cat((pts, norm), dim=-1).cpu().numpy(),
        #     fmt="%.4f")
        # # import ipdb; ipdb.set_trace()
        ##########################################################
        ...
        self.rays_init = GtRays(
            ...
            vpsplanar_mask=self.planar_mask,
            # vpsplanar_mask=None,
            mask=None,
        )
        self.rays = self.rays_init
