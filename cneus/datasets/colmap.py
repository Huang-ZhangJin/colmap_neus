...
class ColmapDataset(DatasetBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        image_list.sort(key=lambda x: int(os.path.basename(x)[:-4].split("_")[1]))
        ...
        self.image_names = []
        intrinsic = np.loadtxt(os.path.join(self.data_root, "intrinsic.txt"))[:3, :3]

        for imgname in tqdm(
            image_list, desc=f"Loading camera parameters and images ^-^ : {split}"
        ):
            self.image_names.append(imgname)
            c2w = np.loadtxt(os.path.join(self.data_root, "pose", f"{imgname[:-4]}.txt"))
            pose_all.append(torch.from_numpy(c2w).float())
            intrinsic_all.append(torch.from_numpy(intrinsic).float())

            rgb = imageio.imread(os.path.join(self.data_root, "images", f"{imgname[:-4]}.png"))
            all_gt.append(torch.from_numpy(rgb))

            if split == "train":
                maskone = imageio.imread(
                    os.path.join(
                        self.data_root, 
                        # "masks", 
                        "u2net_mask",
                ...
                maskone = maskone[..., 0] if len(maskone.shape) == 3 else maskone
                maskone = maskone / 255 
                maskone[maskone>0.5] = 1
                maskone[maskone<=0.5] = 0
        ...
        if self.split == "train":
            assert(patch_size == 1, "Colmap dataset does not support patch mode temporally")
            self.gen_rays_colmap_neus(factor=factor)
    ...
    def gen_rays_colmap_neus(self, factor: int = 1) -> None:
        # Generate rays
        self.factor = factor
