...
def compute_vps_gt(
    ...
    vps_norm: torch.Tensor,  # [n_rays, 6, 3]
    thresh: float = 0.9
) -> Tuple[torch.Tensor]:
    n_rays, n_vps = vps_norm.shape[:2]
    norm = norm[:, None, :].expand(n_rays, n_vps, 3)  # [n_rays, 6, 3]
    cos = nn.CosineSimilarity(dim=2, eps=1e-6)
    cos_sim = cos(vps_norm, norm)  # [n_rays, 6]
    score_map, mmap = torch.max(cos_sim, 1)  # [n_rays]

    mmap_label = mmap[:, None, None].expand(n_rays, 3, 1)  # [n_rays, 3, 1]
    vps_norm = vps_norm.permute(0, 2, 1)  # [n_rays, 3, 6]
    aligned_vps_norm = torch.gather(vps_norm, 2, mmap_label).squeeze()  # [n_rays, 3]
    ...
    return aligned_vps_norm, mmap_mask
